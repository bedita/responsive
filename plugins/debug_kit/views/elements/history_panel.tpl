...
{else}
	{$c = count($content)}{$c} previous requests available
	<ul class="history-list">
		<li>{$html->link('Restore to current request', '#', ['class' => 'history-link', 'id' => 'history-restore-current'])}
		</li>
		{foreach $content as $previous}
			<li>{$html->link($previous.title, $previous.url, ['class' => 'history-link'])}</li>
		{/foreach}
	</ul>
{/if}
